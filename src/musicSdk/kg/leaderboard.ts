...
const getUrl = (p: number, id: string, limit: number) => {
...
const getList = async (bangid: string, page: number, retryNum = 0): Promise<any> => {
  if (++retryNum > 3) throw new Error('try max num')

  const { body } = await httpFetch(getUrl(page, bangid, listDetailLimit)).promise
  if (body.errcode != 0) return getList(bangid, page, retryNum)

  const total = body.data.total
  const listData = filterData(body.data.info)
...
export default {
  ...
  getBoards,
  getList,
  getDetailPageUrl(id: string) {
    const cleanId = id.replace('kg__', '')
    return `https://www.kugou.com/yy/rank/home/1-${cleanId}.html`
  },
}
